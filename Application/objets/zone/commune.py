...
class Commune(Zonage):
    ...
    def __init__(self, id: str, geom_coord: MultiPolygone, nom: str):
        ...
        # super().__init__(self, id= id, geom_coord= geom_coord) TODO bug à
        # corriger
        self.id = id
    ...
    def lien_zone(self, autre_zone: Zonage) -> str:  # pas forcément utilisé
        ...
        """
        # on teste si autre_zone est une parcelle (identifiant à plus de 5
        # caractères)
        if len(autre_zone.id) > 5:
            # ensuite on teste si les polygones (de la parcelle et de la
            # commune) sont contigues (un segment commun)
            if self.geom_coord.test_polyg_contigu(autre_zone.geom_coord):
                # enfin on teste si la parcelle est dans la commune (à l'aide
                # de l'identifiant)
