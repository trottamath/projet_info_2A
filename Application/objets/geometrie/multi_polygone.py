...
class MultiPolygone(AbstractPolygone):
    ...
    def rectangle_circonscrit(self) -> Rectangle:
        ...
        for i in range(1, len(self.liste_polyg)):
            ...
            rectangle = rectangle.union_rectangle(
                autre_rect=poly_ext.rectangle_circonscrit())
        return rectangle

    # AbstractPolygone (récursivité possible si autre_polyg est un
    # multipolygone)
    def test_polyg_contigu(self, autre_polyg: AbstractPolygone) -> bool:
    ...
    # supprimmer la méthode suivante vu que le paramètre autre_polyg de la
    # précédante peut être de type AbstractPolygone
    ...
    def __str__(self) -> str:
        ...
        return "\n".join("{}#".format(
            str(self.liste_polyg[i])) for i in range(len(self.liste_polyg)))
