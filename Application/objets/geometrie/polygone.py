...
class Polygone(AbstractPolygone):
    ...
    def __init__(self, liste_brute: list[list[list[float]]]):
        ...
        for i in range(len(liste_brute)):
            # initialisation : création du segment passant par le 1er et
            # dernier point de la liste des sommets
            liste_segments = [Segment(point1=Point(
                liste_brute[i][-1]), point2=Point(liste_brute[i][0]))]

            # boucle sur la liste des points d'un polygone au sens géométrique
            for j in range(len(liste_brute[i]) - 1):
                liste_segments.append(Segment(point1=Point(
                    liste_brute[i][j]), point2=Point(liste_brute[i][j + 1])))
    ...
    def test_segment(self, autre_segment: Segment) -> bool:
        ...
        """
        # print("test d'appartenance d'un segment au polygone principal
        # (bordure)") # à supprimer à la fin TODO
    ...
    def rectangle_circonscrit(self) -> Rectangle:
        ...
        """
        # print("recherche du rectangle circonscrit") # TODO à supprimer à la
        # fin

        # initialisation sur les coordonnées du 1er point du 1er segment du
        # polygone géométrique extérieur
    ...
    def __str__(self) -> str:
        ...
        return "".join(
            "{}/".format(str(self.liste_poly_geom[0][i].point1)) for i in range(len(self.liste_poly_geom[0])))
