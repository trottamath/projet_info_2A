...
# pour importer la staticmethode pour les chaines de caractères tronquées
...
class ParcelleDAO(metaclass=Singleton):
    ...
    def ajout_parcelle(self, id_parc: str):
        ...
        id_parc : str
            identifiant de la parcelle
        """
        if self.recherche_parcelle(id_parc) == [] or self.recherche_parcelle(
                id_parc) is None:  # la parcelle n'est pas présente dans la table
    ...
    def ajout_liste_parc(self, list_id_parc: list[str]):
        ...
        for id_parc in list_id_parc:
            if self.recherche_parcelle(
                    id_parc) is None or self.recherche_parcelle(id_parc) == []:
