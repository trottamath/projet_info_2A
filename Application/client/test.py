...
# ctime = os.path.getctime("Application/client/data/commune/commune/cadastre-04004-communes.json.gz")
# print(ctime)
...
def create_place():
    ...
    if count > 2:
        for filename in os.listdir(path):
            if time.ctime(os.path.getctime(filename)) < (time.time()- (7*86000)):
                os.remove(filename)
                print('True')
            else : 
                print('False')
