...
class Requete ():
    ...
    # faire une classe abstraite puis une classe par requète pour éviter les
    # if else ? TODO
    def Get_Client(self) -> list[str]:
        ...
        if self.dico_requete["num"] == "1":
            ...
            for id_dep in list_dep:
                ...
                # print(type(autre_list_com))
                list_com = [*list_com, *autre_list_com]  # concaténation

                # print(type(list_com))
                # print("coucou2ter")
                # print(list_com[len(list_com)-1].id) #bug ci-dessus pour
                # certaines communes ????
            list_id_com_contig = []
            # print("coucou3")
            # print(type(list_com))
            ...
            # enregistrement dans la base de données
            # à la place de ParcelleCommuneDAO()
            ...
            requete2_com = Requete(
                dico_requete={
                    "num": "2",
                    "id": id_com,
                    "date": self.dico_requete["date"]})  # recherche des parcelles en limites de cette commune
            ...
            liste_parc_com1 = Instanciation(
                ...
                id1=id_com,
                zonage2="parcelles",
                date=self.dico_requete["date"]).instancier_zonage()  # liste de toutes les parcelles de cette commune
                ...
                if list_id_com_contig != [] and list_id_com_contig is not None:
                    for id_com2 in list_id_com_contig:  # pour chaque commune contigüe
                        ...
                        liste_parc_com2 = Instanciation(
                            ...
                            id1=id_com2,
                            zonage2="parcelles",
                            date=self.dico_requete["date"]).instancier_zonage()  # liste des parcelles de la commune voisine
                        liste_parc_lim2 = []
            ...
            # en cas d'erreur de numéro de requête saisi par l'utlisateur (à
            # supprimer)
            return None

    def Get_or_create(self) -> list[str]:
        if self.Get_DAO() is not None and self.Get_DAO() != []:  # à vérifier TODO
            # print(self.Get_DAO())
            return self.Get_DAO()
        else:
    ...
    # controleur si la requète n'est pas valide par exemple pour un
    # identifient de commune inconnue
